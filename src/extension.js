...
import {Fulu, ExtensionBase, Destroyable, manageSource, omit, getSelf, _} from './fubar.js';
...
class DesktopLyric extends Destroyable {
    ...
    set span(span) {
        this._span = span;
        this._src.play.refreshSource(this._src.play._delegate);
    }

    set playing(playing) {
        this._updateViz();
        if(xnor(playing, this._src.play._delegate)) return;
